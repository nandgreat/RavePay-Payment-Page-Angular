...
@Component({
  selector: 'app-approval-process-flow',
  templateUrl: './approval-process-flow.component.html',
  styleUrls: ['./approval-process-flow.component.scss']
})
export class ApprovalProcessFlowComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get(approvalURL.approvalprocessflow.all)
    .subscribe(success=>{
    ...
    .subscribe(success=>{
      this.ProcessModule = success.data;
    }, error=>{
      console.log(error);
    });

    this.http.get(approvalURL.approvalstage.all)
    .subscribe(success=>{
